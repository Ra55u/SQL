...
-- 1. view tuleb luua koos schemebinding-ga
-- 2. kui on lisafunktsioon select list viitab väljendile ja selle tulemuseks
...
create table ##TestTempTable(Id int, Name nvarchar(20), Gender nvarchar(20))
...
-- 1. After trigger (kustutakse ka FOR triggeriks)
-- 2. Instead of trigger (selmet trigger e selle asemele trigger)
...
-- peale iga töötaja sisestamist tahame teada saada Id-d, päeva ning aega (millal sisestati)
...
insert into Employees values (11, 'Bob', 'Male',3000,  1)
insert into Employees values (10, 'Pam', 'Female', 3500, 3)
insert into EmployeeAudit (11, 10)
...
as begin
	...
	begin
		set @AuditString = ''
		-- selekteerib esimese rea andmed temp tables-st
		...
		@OldDepartmentId = DepartmentId, @OldName = Name
		...
		if(@OldName <> @NewName)
			set @AuditString = @AuditString + ' Name From ' + CAST(@OldName as nvarchar(20))
...
update Employees set Name = 'test123', Salary = 4000
where Id = 2
